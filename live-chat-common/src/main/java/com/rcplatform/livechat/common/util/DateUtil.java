...
public class DateUtil {
	...
	public static Date getDay(Date date){
        String format = new SimpleDateFormat(DATE_FORMAT_DAY).format(date);
        try {
            return new SimpleDateFormat(DATE_FORMAT_DAY).parse(format);
	 ...
	 * @param	hour		增加的小时数
	 */
	public static Date getDatePlusHour( Date inputDate, int hour ) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(inputDate);
	...
	/**
	 * 返回一个日期加上指定天数 sql.date
	 * @param date
	 * @param day
     * @return
     */
	public static java.sql.Date getDateAddDays(java.sql.Date date,Integer day){
		Calendar calendar = Calendar.getInstance();
